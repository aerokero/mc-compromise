...
# Sprinting drains it from full to empty in ~16s; stopping regenerates it after a
...
#
# Tunables:
#   SprintDrainTimer threshold (16)  - ticks between -1 food while sprinting
#   SprintIdleTimer threshold (30)  - delay in ticks after stopping before regen starts
#   SprintRegenTimer threshold (10)  - ticks per +1 food while regenerating
...
execute as @a at @s if predicate main:is_sprinting if score @s SprintDrainTimer matches 1 run effect give @s minecraft:hunger 1 255 true
execute as @a at @s if predicate main:is_sprinting if score @s SprintDrainTimer matches 16.. run scoreboard players set @s SprintDrainTimer 0
...
execute as @a at @s unless predicate main:is_sprinting if score @s SprintIdleTimer matches 30.. run scoreboard players add @s SprintRegenTimer 1
execute as @a at @s unless predicate main:is_sprinting if score @s SprintRegenTimer matches 10.. run scoreboard players set @s SprintRegenTimer 0
execute as @a at @s unless predicate main:is_sprinting if score @s SprintIdleTimer matches 30.. if score @s SprintRegenTimer matches 0 run effect give @s minecraft:saturation 1 0 true
